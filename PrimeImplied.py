...
class PrimeImplied:
    def __init__(self, min_term: list, vars: list) -> None:
        ...
        self._implied_before = self._count_s_min_term()
        self._implied_cur = [[]]
        self._implied = []
        self._until_cur()
    ...
    def _count_s(self, arr: list):
        if len(arr) == 0:
            return [[]]
        temp = [[] for _ in range(self._var_num + 1)]
        for i in arr:
            num_b_i = i.count("1")
            temp[num_b_i].append(i)
        return temp
    ...
    def _have_cur(self):
        arr = []
        mark_arr = copy.deepcopy(self._implied_before)
        for i in range(len(self._implied_before) - 1):
            for j in range(len(self._implied_before[i])):
                for k in range(len(self._implied_before[i + 1])):
                    is_one, one_text = self._one_different(
        ...
        for i in range(len(mark_arr)):
            for j in range(len(mark_arr[i])):
                if not mark_arr[i][j] == 1:
                    self._implied.append(self._implied_before[i][j])
        self._implied_cur = self._count_s(new_arr)
        return bool(new_arr)
...
if __name__ == "__main__":
    # Example usage:
    min_terms = [0, 1, 2, 5, 6, 7, 10, 14, 15]
    variables = ["A", "B", "C", "D"]
    prime_impl = PrimeImplied(min_terms, variables)
    implied_terms = prime_impl.get_implied()
    print(implied_terms)
